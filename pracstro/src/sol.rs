...
impl BaseCatalogObject for Sun {
    fn angdia(&self, d: time::Date) -> Option<time::Angle> {
        Some(time::Angle::from_degrees(0.5333333333) / self.distance(d))
    }

    fn phaseangle(&self, _: time::Date) -> Option<time::Angle> {
        None
    }
}
...
impl CelObj for Planet {
    ...
    fn brightness(&self, d: time::Date) -> f64 {
        5.0 * ((self.distance(d) * self.sun_distance(d)) / self.illumfrac(d).unwrap().sqrt())
            .log10()
            + self.v0
    }
...
impl BaseCatalogObject for Planet {
    fn angdia(&self, d: time::Date) -> Option<time::Angle> {
        Some(self.theta0 / self.distance(d))
    }
    ...
    fn phaseangle(&self, d: time::Date) -> Option<time::Angle> {
        ...
        if (tx * tx + ty * ty + tz * tz).sqrt() < 1.0 {
            ...
            Some(upa + time::Angle::from_degrees(180.0))
        }
    }
}
...
mod tests {
    ...
    fn test_phase() {
        assert_eq!(
            VENUS
                .illumfrac(time::Date::from_calendar(
                    2025,
                    03,
                    24,
                    time::Angle::default()
